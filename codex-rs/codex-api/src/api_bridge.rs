            ...
            TransportError::Http {
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.matches_kind("usage_limit_reached") {
                        ...
                        } else if err.error.matches_kind("usage_not_included") {
                            return CodexErr::UsageNotIncluded;
                        } else if err.error.matches_kind("insufficient_quota") {
...
impl UsageErrorBody {
    fn matches_kind(&self, expected: &str) -> bool {
        self.error_type.as_deref() == Some(expected) || self.code.as_deref() == Some(expected)
    }
}
