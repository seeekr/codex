...
pub(crate) async fn run_turn(
    ...
    loop {
        ...
        // Pending user input must be durable before any cancellable enrichment work. Otherwise a
        // Stop arriving while skills, plugins, or apps are resolved could silently discard an
        // already accepted steer.
        // Capture once so pending-input enrichment, advertised tools, and tool calls share one
        // request view.
        let step_context = match next_step_context.take() {
            Some(step_context) => step_context,
            None => sess.capture_step_context(Arc::clone(&turn_context)).await,
        };
        let pending_enrichment = if pending_input
            .iter()
            .any(|item| matches!(item, TurnInput::UserInput { .. }))
        {
            let Some(enrichment) = build_skills_and_plugins(
                &sess,
                step_context.as_ref(),
                &pending_input,
                &cancellation_token,
        ...
        if let Some((injection_items, explicitly_enabled_connectors)) = pending_enrichment {
            sess.merge_connector_selection(explicitly_enabled_connectors)
                .await;
            for response_item in injection_items {
                sess.record_conversation_items(&turn_context, std::slice::from_ref(&response_item))
                    .await;
